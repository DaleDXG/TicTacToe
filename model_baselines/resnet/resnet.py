...
# import tf.keras.optimizers.experimental.SGD as SGD
...
class Gen_Model():
	...
	def write(self, game, version):
		# 'model_resnet_' + agents[0].name + '_ep' + str(num_episode) + '.h5'
		self.model.save('model_resnet_v' + version + '.h5')

	def read(self, game, run_number, version):
		return load_model('model_resnet_v' + version+ '.h5', custom_objects={'softmax_cross_entropy_with_logits': util.softmax_cross_entropy_with_logits})
	...
	def printWeightAverages(self):
		...
		for i, l in enumerate(layers):
			try:
				x = l.get_weights()[0]
				util.logger_model.info('WEIGHT LAYER %d: ABSAV = %f, SD =%f, ABSMAX =%f, ABSMIN =%f', i, np.mean(np.abs(x)), np.std(x), np.max(np.abs(x)), np.min(np.abs(x)))
			except:
				pass
		util.logger_model.info('------------------')
		for i, l in enumerate(layers):
			try:
				x = l.get_weights()[1]
				util.logger_model.info('BIAS LAYER %d: ABSAV = %f, SD =%f, ABSMAX =%f, ABSMIN =%f', i, np.mean(np.abs(x)), np.std(x), np.max(np.abs(x)), np.min(np.abs(x)))
			except:
				pass
		util.logger_model.info('******************')
...
class Residual_CNN(Gen_Model):
	...
	def _build_model(self):
		...
		model.compile(loss={'value_head': 'mean_squared_error', 'policy_head': util.softmax_cross_entropy_with_logits},
			# optimizer=SGD(lr=self.learning_rate, momentum = Config.MOMENTUM),
			optimizer=Adam(learning_rate=self.learning_rate), #, momentum = Config.MOMENTUM),
