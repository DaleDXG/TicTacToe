...
from model_baselines.mcts import mcts
# from util import softmax_cross_entropy_with_logits
import util
...
class Agent():
	...
	def simulate(self):
		...
		util.logger_mcts.info('ROOT NODE...%s', self.mcts.root.state.id)
		# self.mcts.root.state.render(util.logger_mcts)
		util.logger_mcts.info('CURRENT PLAYER...%d', self.mcts.root.state.playerTurn)

		##### MOVE THE LEAF NODE
		leaf, value, done, breadcrumbs = self.mcts.moveToLeaf()
		leaf.state.render(util.logger_mcts)
	...
	def evaluateLeaf(self, leaf, value, done, breadcrumbs):
		...
		if done == 0:
			...
			util.logger_mcts.info('PREDICTED VALUE FOR %d: %f', leaf.state.playerTurn, value)
			...
			util.logger_mcts.info('GAME VALUE FOR %d: %f', leaf.playerTurn, value)
	...
	def changeRootMCTS(self, state):
		util.logger_mcts.info('****** CHANGING ROOT OF MCTS TREE TO %s FOR AGENT %s ******', state.id, self.name)
